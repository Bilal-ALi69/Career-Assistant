...
export default function DatePicker({ value, onChange, dark, tokens, placeholder = "Select date" }) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <>
          {/* Desktop: dropdown anchored under the trigger (never clipped) */}
          <div
            className={cx(
              "hidden md:block absolute z-50 mt-1.5 w-[280px] rounded-xl border p-3",
              "animate-drop-in",
              dark
          ...
          {/* Mobile: viewport-locked bottom sheet */}
          {createPortal(
            <div className="md:hidden fixed inset-0 z-[100]" onMouseDown={close} data-accent={accentAttr()}>
              ...
              >
                ...
                <div className="mt-1 max-h-[60vh] overflow-y-auto px-3 pb-2">
                  {renderPanel()}
                </div>
              </div>
            </div>,
